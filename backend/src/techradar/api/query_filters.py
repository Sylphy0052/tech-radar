...
# 黙って結果をすり替えない）。
#
# この経路でも実際に 500 になることは確認した。上限を入れる前に `offset=10**19` を
# 投げると、`... LIMIT %(param_1)s::INTEGER OFFSET %(param_2)s::BIGINT` のバインドで
# 同じ `DataError` になる。一方、上限値 10**8 が安全側にあることは `MAX_PAGE_NUMBER`
# のコメントに書いた実測から従う（OFFSET は 2**63 - 1 まで例外にならず、10**8 は
# その内側にある）。こちらは測り直していない。
#
