...
# 要約と分類には冒頭で足りることが多い。
#
# 実データ（記事 194 件）で測り直したうえで 12000 を維持している。応答時間は本文長に
# ほぼ依存せず（8 倍にして +17%、実行ごとのばらつきに埋もれる）、切り捨てによる解析結果の
# 差も同じ入力を 2 回解析したときのばらつきと同程度だった。この上限では 15.5% の記事が
# 切り捨てられる。計測の詳細と再現手順は docs/adr/0004-analysis-body-limit.md にある。
