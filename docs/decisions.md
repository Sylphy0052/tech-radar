...
| 一度の表示件数 | 1 ページ 20 件（`config/scoring.yaml` の `limits.default_page_size`） |
| ページング | 番号付きページ（`page` / `limit`）。総件数と総ページ数を API が返す（Issue #90） |
| フィードの絞り込み | 検索語（原文タイトル / 訳タイトル / 日本語要約への部分一致）・topics・technologies・公開日の範囲・情報源ドメイン・対象期間。条件は URL クエリに載せる（Issue #90） |
| フィードの対象期間 | 1〜180 日で指定でき、既定は `scoring.yaml` の `freshness.max_age_days`（7 日）。候補の絞り込みにだけ使い、freshness スコアの減衰基準は変えない（Issue #90） |
...
| 巡回先の自動発見 | 登録記事のドメインから RSS/Atom を探し、見つかったものを巡回先へ加える。確認 UI は置かず、発見した時点で有効にする（Issue #93、下記） |
...
### 巡回先の自動発見をどこまで自動にするか（Issue #93）

保存先は `source_registry` を流用せず、新しい `discovered_feeds` テーブルにする。
`source_registry` は「公式ソースかどうか」を判定して権威スコアを与えるための規則の
置き場であり、巡回先の一覧ではない。両者を混ぜると、自動発見したドメインに権威スコアの
意味まで付いてしまう。

発見したフィードは確認 UI を挟まず、その場で `enabled=True` にする。1 名運用で承認する
相手が自分しかいないうえ、承認待ちの一覧を見に行く導線を作ること自体が手間になるため。
質の低いフィードが混ざったときは、そのフィード由来の記事へ Bad を付ければ情報源選好が
下がる（上記の `source_authority` 係数）ので、取り込みの段階で止めなくても実害は収束する。

暴走を止めるのはコード定数の上限だけにする（`collectors/discovery.py`）。1 回の巡回で
発見を試みるドメイン数、自動追加できるフィードの総数、失敗したドメインの再試行間隔、
1 ドメインから試すフィード候補数の 4 つ。`config/feeds.yaml` へは置かない。あちらは人間が
キュレーションする巡回先の宣言用ファイルであり、アルゴリズムのパラメータの置き場ではない。

候補にするのは、トップページと同じホスト（またはそのサブドメイン）の https URL に限る。
`<link rel="alternate">` の `href` には任意の第三者 URL を書けるため、制限しないと外部
サイトの HTML が「攻撃者の選んだ URL を恒久的な巡回先として登録させる」ことができてしまう。
内部アドレスへの到達自体は `fetcher.ssrf.validate_url` が止めるが、外部サイトの選択までは
止めない。
