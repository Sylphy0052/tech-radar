...
これらは実データが無いと判断できないため保留していた。Issue #73 の時点（2026-08-12）では関心記事の embedding が 1 件も無く、クラスタは「対象データがありません」、フィード枠は strong_interest の 55 件が全て補充由来という状態だった。
...
**`min_clusters` と `min_articles_per_cluster` は、この母数では死んでいる。** 「3. min_articles_per_cluster の感度」のとおり結果に一切現れない。ただし死ぬ理由は 2 つで別物である。

- `min_clusters` は `_cluster_count` に登場しない。クラスタ数は `capacity_based` を `max_clusters` で挟んで決まるだけで、この値は「記事が `min_clusters * min_articles_per_cluster = 6` 件に達すれば自動的に満たされる目安」にすぎない
- `min_articles_per_cluster` が結果を決めるのは、`capacity_based` が `max_clusters` に届かない規模、つまり記事が `max_clusters * min_articles_per_cluster`（既定では 24 件）に満たないうちだけである。69 件はこれを大きく上回る

記事が `min_articles_per_cluster`（3 件）に満たないときに無理に分けず 1 クラスタへ丸める挙動は、Issue #15 の自己レビューで確定済みである。いずれも実データで否定する材料が無いため据え置く。
