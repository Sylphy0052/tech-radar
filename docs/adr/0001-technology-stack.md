...
MVP では認証を設けない。将来のマルチユーザー化を妨げないため `user_id` を持たせる。
付与する範囲は当初「全テーブル」としていたが、記事本体・公式ソースレジストリ・ジョブキュー・構造化ログはユーザー横断で共有するものであり、`recommendations` も `run_id` 経由で `recommendation_runs.user_id` へ辿れる。実装はこれらを対象外としており、ユーザー固有のデータを持つテーブルに限る形へ訂正した。内訳は [decisions.mdの認証節](../decisions.md#認証) を参照する。
