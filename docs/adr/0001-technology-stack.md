...
- **DNS リバインディングは防いでいない**（Issue #3）。記事取得では接続の直前に名前解決を行い、
  ...
  次の理由から MVP では受容する。ただし残存リスクとして Issue #21 で追跡する。
  - 登録される URL は利用者本人が入力するか、公式 RSS など巡回対象から得たもの
  - ローカル環境で動くため、外部ネットワークから API へ到達する経路がない
  - 到達できたとしても、応答が外部ネットワークへ送信されることはない
  ...
  マルチユーザー化や外部公開を行う際は、接続先 IP を固定する専用トランスポート（Issue #21）が必須になる。
