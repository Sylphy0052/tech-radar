...
MVP では認証を設けない。ただしユーザー固有のデータを持つテーブルには `user_id` を持たせ、将来のマルチユーザー化を妨げない。記事本体・公式ソースレジストリ・ジョブキュー・構造化ログはユーザー横断で共有するため持たせない。内訳は [decisions.md の認証節](../decisions.md) を参照する。
