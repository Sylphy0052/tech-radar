...
上の結論は、試したポリシーキーと、この1環境（Linux / WSL、CLI 2.1.227）について言えることに限られる。
次は確かめていないため、`--tools ""` 以外の防御層がポリシーから崩されないことまでは保証しない。

- `apiKeyHelper` — 認証情報の取得に任意コマンドを実行する設定キー。hooks と同様の実行経路になりうる
- ポリシーファイルへ直接書く `mcpServers` や `enableAllProjectMcpServers` — `--strict-mcp-config` はコマンドライン側の
  指定を排他にするフラグであり、ポリシー由来のサーバ定義まで排除するかは確認していない
- `--disable-slash-commands`（Skills 無効化）をポリシー側から再有効化できるか
- 環境変数の許可リストを迂回する経路
- macOS / Windows での挙動、および他の CLI バージョン
- 将来ポリシーへ追加されるキー

これらの実測は Issue #66 で扱う。CLI を更新したときは、この節も含めて再検証する。
...
- **管理者ポリシーが配布されたホストでは、hooks 経由の実行経路が開く**。上記 1. のとおり管理者ポリシーはコマンドライン引数より優先され、CLI 側にこれを無効化する手段は無い。`--bare` よりさらに広範に無効化する `--safe-mode` でも "Admin-managed (policy) settings still apply." と明記されている。
  ...
  影響範囲は Issue #56 で実測した（CLI 2.1.227、詳細は上記「管理者ポリシーはコマンドライン引数を上書きしない」）。**主防御の `--tools ""` は維持される**——ポリシーで `permissions.allow` にツールを列挙しても、`defaultMode` を `bypassPermissions` にしても、ツールは動かなかった。ポリシー側に `--tools` へ相当するキーが見当たらないためで、`permissions` はツールの**存在**ではなく**呼び出しの可否**を扱う別の層にある。一方 **hooks はポリシーから実行される**ことが確認された（`managed-settings.d/` の drop-in を含む）。hooks は任意コマンドを実行できるため、この1点だけで隔離は無意味になる

  ただし**実測が確かめたのは試したポリシーキーとこの1環境に限られる**。`apiKeyHelper` やポリシーへ直接書く `mcpServers`、Skills の再有効化、macOS / Windows での挙動は確認していない（上記「この実測が確かめていない範囲」、Issue #66）。「穴は hooks だけ」と読み替えないこと
